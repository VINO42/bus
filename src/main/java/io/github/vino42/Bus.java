...
public class Bus implements Runnable {
    ...
    public void run() {
        ...
        System.out.println("线程号："+Thread.currentThread().getId()+"车号：" + this.getBusNumber() + (this.getDirection() == 1 ? "正向" : "反向") + "当前公交开始运行| 车号：" + this.getBusNumber());
    ...
    private void fanXiang() throws InterruptedException {
        ...
        while (fan.hasNext()) {
            ...
            System.out.println("线程号："+Thread.currentThread().getId()+"反向行驶" + "车号：" + this.getBusNumber() + "停靠站台： " + currentStation.getIndex() + "站台等待乘客数: "
                    + currentStation.getWaitingPassenger().size() + "车上乘客数：" + this.getPassengers().size());
            //到站下车上车
            daozhan(currentStation);
            System.out.println("线程号："+Thread.currentThread().getId()+"车号：" + this.getBusNumber() + "下车上车完毕 继续出发" + "车上乘客数:" + this.getPassengers().size());
            ...
            if (forwardDirectTime != null && currentStation.getNext() != null) {
                ...
                System.out.println("线程号："+Thread.currentThread().getId()+"车号：" + this.getBusNumber() + (this.getDirection() == 1 ? "正向" : "反向") + "运行到下一站| 下一站 站号" + currentStation.getNext().getName() + "车上乘客数:" + this.getPassengers().size());
            } else {
                //到终点站了 直接跳出循环进行调向
                break;
            }
        }
        System.out.println("线程号："+Thread.currentThread().getId()+"车号：" + this.getBusNumber() + (this.getDirection() == 1 ? "正向" : "反向") + "当前公交到达反向终点，开始正向运行| 车号：" + this.getBusNumber() + "车上乘客数:" + this.getPassengers().size());
        this.setDirection(1);
        zhengXiang();
    }
    ...
    private void zhengXiang() throws InterruptedException {
        ...
        while (it.hasNext()) {
            ...
            System.out.println("线程号："+Thread.currentThread().getId()+"正向行驶" + "车号：" + this.getBusNumber() + "停靠站台： " + currentStation.getIndex() + "站台等待乘客数: "
                    + currentStation.getWaitingPassenger().size() + "车上乘客数：" + this.getPassengers().size());
        ...
        System.out.println("线程号："+Thread.currentThread().getId()+"车号：" + this.getBusNumber() + (this.getDirection() == 1 ? "正向" : "反向") + "当前公交到达正向终点，开始反向运行| 车号：" + this.getBusNumber() + "车上乘客数:" + this.getPassengers().size());
        this.setDirection(2);
        fanXiang();
    }
    ...
    private void daozhan(Station currentStation) throws InterruptedException {
        ...
        if (brokenFlag == 1) {
            //公交故障拉
            System.out.println("线程号："+Thread.currentThread().getId()+"车号：" + this.getBusNumber() + " 故障！全体乘客下车,乘客上车站改为当前站 " + currentStation.getIndex());
            passengerStrategy.busBroken(currentStation, this);
            System.out.println("线程号："+Thread.currentThread().getId()+"车号：" + this.getBusNumber() + " 故障！当前修车30秒！ 在站台: " + currentStation.getIndex());
